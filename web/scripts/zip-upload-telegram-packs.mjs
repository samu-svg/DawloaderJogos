/**
 * Zip each Xbox 360 game folder under Telegram Desktop packs, upload to R2,
 * then delete the local zip AND the original game folder (saves disk space).
 * Re-run skips keys already on R2 and still removes the local folder if present.
 *
 *   node --env-file=.env.local scripts/zip-upload-telegram-packs.mjs
 */
...
const TELEGRAM_DIR = "C:\\Users\\doura\\Downloads\\Telegram Desktop";
...
/** Directories whose root contains default.xex are game folders. */
async function discoverGames(rootDir, games, depth = 0) {
  if (depth > 5) return;
  ...
  if (hasXex) {
    games.push({ name: path.basename(rootDir), absPath: rootDir });
    return;
  }

  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
    await discoverGames(path.join(rootDir, entry.name), games, depth + 1);
  }
}
...
async function main() {
  await access(TELEGRAM_DIR);
  const s3 = client();
  await mkdir(ZIP_DIR, { recursive: true });

  const games = [];
  const topEntries = await readdir(TELEGRAM_DIR, { withFileTypes: true });
  for (const entry of topEntries) {
    if (!entry.isDirectory()) continue;
    await discoverGames(path.join(TELEGRAM_DIR, entry.name), games);
  }
  ...
  const uniqueGames = [...byName.values()].sort((a, b) =>
    a.name.localeCompare(b.name, "pt-BR"),
  );

  const summary = { uploaded: [], skipped: [], failed: [] };
  console.log(`Found ${uniqueGames.length} game folders in ${TELEGRAM_DIR}`);

  for (let i = 0; i < uniqueGames.length; i += 1) {
    const { name: folderName, absPath } = uniqueGames[i];
    const key = r2Key(folderName);
    const label = `[${i + 1}/${uniqueGames.length}] ${folderName}`;
    console.log(`\n${label}`);
